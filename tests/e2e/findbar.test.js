...
const nativeRpc = (method, params = {}, id = `findbar-rpc-${Date.now()}`) => `
  new Promise((resolve) => {
    window.cefQuery({
...
test('findbar RPC rejects unknown schema fields',
  ...
  async () => {
    ...
    try {
      ...
      assert.equal(response.id, 'findbar-extra-param');
