...
const splitUnreleasedSection = (text) => {
  const unreleasedHeading = /^## \[Unreleased\]\s*$/m
  const headingMatch = unreleasedHeading.exec(text)
  if (!headingMatch) {
    die(`${changelogPath} is missing a '## [Unreleased]' section`)
  }
  ...
  const sectionEnd =
    nextHeadingOffset === -1 ? text.length : sectionStart + nextHeadingOffset

  return {
    before: text.slice(0, headingMatch.index),
    body: text
      .slice(sectionStart, sectionEnd)
      .replace(/^\n+/, '')
      .replace(/\s+$/, ''),
    after: text.slice(sectionEnd),
  }
}

const releaseChangelog = (text, version, releaseDate) => {
  const { before, body, after } = splitUnreleasedSection(text)
  const unreleasedSection = '## [Unreleased]\n'
  const versionSection = [`## [${version}] - ${releaseDate}`, body]
    .filter(Boolean)
    .join('\n\n')
  const rebuilt = [
    before.trimEnd(),
...
if (dirty) {
  console.error(
    `${COLOR.red}✗ working tree has uncommitted changes:${COLOR.reset}`,
  )
  console.error(dirty)
...
const cmakeMatch = cmakeText.match(
  /project\(\s*otf-browser\s+VERSION\s+(\d+\.\d+\.\d+)/,
)
if (!cmakeMatch)
  die(`could not find 'project(otf-browser VERSION X.Y.Z ...)' in ${cmakePath}`)

console.log(`${COLOR.dim}Current package.json:  ${pkg.version}${COLOR.reset}`)
console.log(
  `${COLOR.dim}Current CMakeLists.txt base: ${cmakeMatch[1]}${COLOR.reset}`,
)
...
let baseVersion
while (true) {
  baseVersion = await ask(
    `${COLOR.cyan}Version${COLOR.reset} (X.Y.Z)`,
    cmakeMatch[1],
  )
  if (semverRe.test(baseVersion)) break
  console.error(
    `${COLOR.red}  → '${baseVersion}' isn't X.Y.Z; try again${COLOR.reset}`,
  )
}
...
if (isPre) {
  while (true) {
    const choice = (
      await ask(
        `${COLOR.cyan}Pre-release type${COLOR.reset} (1) alpha  (2) beta`,
        '1',
      )
    ).toLowerCase()
    if (choice === '1' || choice === 'alpha') {
      prereleaseTag = 'alpha'
      break
    }
    if (choice === '2' || choice === 'beta') {
      prereleaseTag = 'beta'
      break
    }
...
if (isPre) {
  ...
  const answer = await ask(
    `${COLOR.cyan}${prereleaseTag} counter${COLOR.reset}`,
    suggested,
  )
  const parsed = Number.parseInt(answer, 10)
  if (!Number.isInteger(parsed) || parsed < 1)
    die(`'${answer}' isn't a positive integer`)
...
try {
  ...
  if (
    e.status !== 1 &&
    !String(e.message).includes('not a valid object name')
  ) {
...
console.log(
  `  ${COLOR.dim}package.json version  →${COLOR.reset} ${fullVersion}`,
)
console.log(
  `  ${COLOR.dim}CMakeLists.txt base   →${COLOR.reset} ${baseVersion}  ${COLOR.dim}(CMake VERSION doesn't accept hyphens; CI overrides with full string via OTF_VERSION env)${COLOR.reset}`,
)
console.log(
  `  ${COLOR.dim}CHANGELOG.md          →${COLOR.reset} ${fullVersion} (${releaseDate})`,
)
console.log(`  ${COLOR.dim}Git tag               →${COLOR.reset} ${tagName}`)
console.log(
  `  ${COLOR.dim}Push target           →${COLOR.reset} origin ${branch}  (with --follow-tags)`,
)
console.log('')
...
const newChangelog = releaseChangelog(changelogText, fullVersion, releaseDate)
writeFileSync(changelogPath, newChangelog)
console.log(`${COLOR.green}✓${COLOR.reset} wrote ${changelogPath}`)
...
run(`git add ${pkgPath} ${cmakePath} ${changelogPath}`)
...
try {
  execSync(
    `git diff --cached --quiet -- ${pkgPath} ${cmakePath} ${changelogPath}`,
  )
  needsCommit = false
} catch (_) {
...
if (needsCommit) {
  ...
  console.log(
    `${COLOR.dim}(version files already at ${fullVersion} — tagging current commit instead of creating an empty one)${COLOR.reset}`,
  )
}
...
console.log(
  `${COLOR.dim}  Watch the CI build at:  $(git remote get-url origin)/actions${COLOR.reset}`,
)
